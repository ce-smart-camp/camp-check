<template>
  <v-container grid-list-xl>
    <v-layout wrap>
      <v-flex v-for="qus in questions" :key="qus.key" xs12>
        <v-card>
          <v-card-text>
            <div class="my-3">
              <p v-if="typeof qus.text === 'string'">{{ qus.text }}</p>
              <template v-for="sub in qus.text" v-else
                >{{ sub }}<br :key="sub"
              /></template>
            </div>
            <v-textarea
  ...
  data() {
    return {
      questions: [
        ...
        {
          text: `9. มีวัตถุดิบ 10อย่าง โดยวัตถุดิบทุกอย่างสามารถนำไปนึ่ง ลวก ย่าง หรือทอด
